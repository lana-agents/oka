...
/-- **A presented algebra is finitely presented over `ℂ`**, and it is structural rather than a
hypothesis: `ComplexAnalytic.Presentation` carries `n : ℕ` variables and `k : ℕ` relations, so
`ComplexAnalytic.presentationIdeal` is the span of the range of a map out of `Fin k` and is
finitely generated by `Submodule.fg_span` of a finite set.

**This is what "of finite type over `ℂ`" asks of a member of an admissible cover**, and stating it
is the only part of that condition this repository did not have — see the admissibility section of
`Oka/Analytification/SpecAffineCover.lean`'s module docstring, and taxis #1329 for the measurement
that put the other two parts on `master` already.

Nothing below or in the cover files consumes it; it is stated because the condition it discharges
is quoted in prose in four files and was discharged by none. -/
instance finitePresentation_presentedAlgebra
