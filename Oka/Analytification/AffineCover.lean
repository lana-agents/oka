...
* **Any statement that a gluing is not affine.** The two instances of this construction —
  `OkaTest/AffineCover.lean`, three copies of the node along the punctured axis, and
  `OkaTest/ProjectiveLine.lean`, two copies of the affine line along `D(z)` by `z ↦ 1/z` — each
  exhibit a space covered by their members and equal to none of them. Neither exhibits a space
  that is not the analytification of *some* presentation, which is a much stronger claim and
  needs an invariant nothing here computes.
