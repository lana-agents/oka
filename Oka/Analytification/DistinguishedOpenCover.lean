...
* **Appending to `Oka/Analytification/DistinguishedOpen.lean` is cheap at that file and not at
  ...
  `scripts/module_graph.py --downstream` counts **157** modules downstream of it, and each of
...
either file gains an import and the 157 moves whenever anything imports `DistinguishedOpen`. **Two
of those 157 are this push's own** — this module, which imports `DistinguishedOpen` directly, and
`OkaTest.DistinguishedOpenCover`, which reaches it through `import Oka` — so the same command at
the parent commit returns **155**, and that is the number of consumers the append this bullet
declines would have found already standing. Both runs are written down because the gap between
them is the shape of figure this section is most likely to get wrong: one measured on the base and
then written into a sentence that pins it to the head. A later reader should re-run them rather
than quote them.
