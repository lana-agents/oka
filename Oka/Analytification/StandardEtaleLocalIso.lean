...
* **No `StandardEtalePair` is constructed *here*, and this bullet's own count of what is
  ...
  the `cond` route through the two theorems below is checked there and not by nothing.
  `ComplexAnalytic.sqSubOnePair` (`OkaTest/OpenBaseFiniteness.lean`) and
  `ComplexAnalytic.sqrtCoverPair` (`OkaTest/StandardEtaleBaseWitness.lean`) are two more, the
  second over an arbitrary `ℂ`-algebra. **What survives is the placement and it is what the bullet
  is for**: the two theorems below inherit that hypothesis rather than discharging it, and this
  file adds no new unwitnessed one. `OkaTest/StandardEtaleAnalytification.lean` exercises the
