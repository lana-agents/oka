...
**First, the declarations named after separatedness.** At `ddcc4c6`, the commit this module is cut
...
paragraph closing this section is the re-run at `ddcc4c6`, after the third. Fifteen commits
...
**And re-run once more at `ddcc4c6`, the base this module is now cut from, which is one commit past
`280bb67` and the cheapest of the three re-cuts to account for.**
...
`Oka/AnalyticSpace/SeparatedFiniteEtaleLimits.lean:249` with its application at `:255`. **Nothing
here is a figure carried from a commit this module is no longer cut from.**
