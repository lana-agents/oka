...
over a Hausdorff preconnected base the two conditions **agree**, so the decomposition theorem there
is a statement about the topological connected components after all. taxis #2082 is the filing.
...
script for, and *abbreviated citations, not counted* is flat at **32, four of them dotted**, and
*advertised from another file* flat at **85**: no head of the list above is another module's
declaration and no citation in that section is a suffix of one. `scripts/check_docstring_names.py`
goes **17672 → 17737** backticked names (4313 → 4325 distinct) and **227 → 265** elided citations
(136 → 148), **0 unresolved at both ends**, **6** resolving under more than one namespace at both,
