...
- `ComplexAnalytic.AnalyticSpace.ext'`: **two analytic spaces with the same underlying locally
  ...
  **Its only consumer at the commit that adds it is `ComplexAnalytic.restrict_eq_ofCutOut`** in
  `Oka/AnalyticSpace/OpenSubspace.lean`, by `git grep` for the name over `Oka/` and `OkaTest/`.
...
this was written for, and the only one. -/
