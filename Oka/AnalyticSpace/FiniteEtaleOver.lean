...
A draft instead carried the difference in a `CategoryTheory.Limits.IsColimit`-shaped `Eq.trans`
and rewrote on the far side of it. **That fails**, and the failure is worth recording because it is
not about the mathematics: `rw` reports that it cannot find `?f ≫ ?g ≫ ?h` in a goal whose
left-hand side has become the cocone's diagram read at
`CategoryTheory.Limits.WalkingPair`, and the error under it is an application type mismatch in
which the structure map's type is spelled through `CategoryTheory.Functor.id` on one side and
through `CategoryTheory.Functor.const` at the cocone's point on the other. That is the seam
`ComplexAnalytic.AnalyticSpace.FiniteEtaleOver.sigma`'s docstring records for its `prop` field, met
here in the target of a rewrite rather than in an instance argument.
