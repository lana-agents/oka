...
written, and the sentence that module opens with — *no statement of this repository at the commit
that adds that instance consumes the class* — had become true of it one level up.
...
cut from `import Oka` brings **5536** modules and at the commit that adds it **5537**, by a set
...
elaborating the base `Oka.lean` at this checkout, which imports 5535 modules and is one short of
...
equation lemma, match lemma or congruence lemma — and the dump total moves **4968 → 4978**, the
other six being `OkaTest/FundamentalGroup.lean`'s. `scripts/DumpEnvNames.lean` moves
**338238 → 338250**, which is those twelve declarations and the two modules minus nothing.

`scripts/guard_coverage.py` moves guards under `OkaTest/Axioms/` **1997 → 2001**, all four in
`OkaTest/Axioms/Morphisms.lean`; advertised **1493 → 1496** in **228 → 229** files; in both
**1351 → 1354**; and guarded and advertised nowhere **646 → 647**, the one being this file's
...
`scripts/check_docstring_names.py` goes **17845 → 17902** backticked names
(**4348 → 4357** distinct) and **306 → 325** elided citations (**155 → 159** distinct), **0
unresolved at both ends**, **6** resolving under more than one namespace at both, and **237**
dotless at both.
...
* **`SimplyConnectedSpace` is Mathlib's and is not what this file's name refers to.**
  ...
  closure already — it is one of the **5536** modules `import Oka` brings at the commit this file
