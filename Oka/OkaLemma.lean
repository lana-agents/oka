...
- `LocalOkaRing.fromPolynomialAlg`: `LocalOkaRing.fromPolynomial` as a morphism of
  `LocalOkaRing (Fin n)`-algebras.
- `polyIncl` and `polyInclPi`: a polynomial (resp. a row of polynomials) of bounded degree
  ...
  variables.
- `kDeg d F`: the relations of bounded degree `d`, as a module over the germs in `n`
  variables.
- `kkDeg d F`: the submodule of all relations generated by `kDeg d F`.
...
/-- `LocalOkaRing.fromPolynomial` as a morphism of `LocalOkaRing (Fin n)`-algebras: a
polynomial in one variable over the germs in `n` variables is a germ in `n + 1` variables,
`LocalOkaRing`-linearly in its coefficients. The underlying ring homomorphism is the one
`LocalOkaRing.fromPolynomial` already provides; only the `commutes'` field is new, and it
holds by `rfl` because the algebra structure is defined through that same map. -/
...
/-! ### Polynomials of bounded degree as germs

Oka's bounded degree lemma compares two modules of relations: relations whose entries are
polynomials of degree `< d` over the germs in `n` variables, and arbitrary relations over the
germs in `n + 1` variables. `polyIncl` and `polyInclPi` are the maps along which the first is
read inside the second, one entry at a time and a whole row at a time respectively. Both are
`LocalOkaRing (Fin n)`-linear — that is the point of the construction, since it is what makes
the relations of bounded degree a *finitely generated* module and drives the induction on `n`.
-/

/-- A polynomial of degree `< d` over the germs in `n` variables, read as a germ in `n + 1`
variables. This is `LocalOkaRing.fromPolynomialAlg` restricted to `Polynomial.degreeLT`, as a
`LocalOkaRing (Fin n)`-linear map. -/
...
/-- A row of `p` polynomials of degree `< d` over the germs in `n` variables, read entrywise
as a row of `p` germs in `n + 1` variables; the `p`-fold product of `polyIncl`. -/
...
/-- The relations of bounded degree between `F₁, …, F_p`: those rows of polynomials of degree
`< d` over the germs in `n` variables which, read as germs in `n + 1` variables by
`polyInclPi`, are a relation between the `F_j`.

This is a submodule over the germs in **`n`** variables, not in `n + 1`; that is the whole
point of Oka's bounded degree lemma, and is what lets the inductive hypothesis apply to it.
`kkDeg` below is the submodule of *all* relations that it generates, and the lemma says the
two describe the same relations. -/
noncomputable def kDeg {p : ℕ} (d : ℕ) (F : Fin p → LocalOkaRing (Fin (n + 1))) :
    Submodule (LocalOkaRing (Fin n)) (Fin p → (LocalOkaRing (Fin n))[X]_d) :=
  LinearMap.ker ((linOfFun F).restrictScalars _ ∘ₗ
    polyInclPi)

/-- The submodule of relations between `F₁, …, F_p` over the germs in `n + 1` variables
generated by the relations of bounded degree `kDeg d F`.

The inclusion `kkDeg d F ≤ LinearMap.ker (linOfFun F)` is immediate from the definition of
`kDeg` — it is `oka_lemma_weierstrass_rhs_containedIn_lhs`. Oka's bounded degree lemma
(`oka_lemma_weierstrass`, `oka_lemma`) is the converse, and is the hard direction. -/
noncomputable def kkDeg {p : ℕ} (d : ℕ) (F : Fin p → LocalOkaRing (Fin (n + 1))) :
    Submodule (LocalOkaRing (Fin (n + 1))) (Fin p → LocalOkaRing (Fin (n + 1))) :=
  Submodule.span (LocalOkaRing (Fin (n + 1)))
    (Submodule.map polyInclPi (kDeg d F)).carrier
...
theorem oka_lemma_weierstrass_rhs_containedIn_lhs (p : ℕ) (d : ℕ)
    ...
    Submodule.span _ (Submodule.map polyInclPi (kDeg d F')).carrier ≤
  ...
  simp only [kDeg, SetLike.mem_coe, LinearMap.mem_ker, LinearMap.coe_comp,
...
theorem oka_lemma (p : ℕ) (d : ℕ)
    ...
    LinearMap.ker (linOfFun F') =
      Submodule.span (LocalOkaRing (Fin (n + 1)))
        (Submodule.map polyInclPi (kDeg d F')).carrier := by
  classical
  apply le_antisymm
  · rcases Nat.eq_zero_or_pos p with rfl | hp
    ...
    have hHmem : H ∈ Submodule.span (LocalOkaRing (Fin (n + 1)))
        (Submodule.map polyInclPi (kDeg d fun j ↦ polyIncl (F j))).carrier := by
      ...
      refine ⟨Hp, ?_, hHpH⟩
      simp only [kDeg, LinearMap.mem_ker, LinearMap.coe_comp, LinearMap.coe_restrictScalars,
...
theorem oka_lemma_weierstrass_lhs_containedIn_rhs (p : ℕ) (d : ℕ) (m : ℕ)
    ...
    letI F' (j : Fin p) : LocalOkaRing (Fin (n + 1)) :=
      LocalOkaRing.fromPolynomial (F j)
    LinearMap.ker (linOfFun F') ≤ kkDeg m F' := by
  ...
  have hH : (H ∈ kkDeg m F') := by
    ...
    simp only [hHpH]
    exact hHrel
  have hE' : ∀ (j : Fin p), (E' j ∈ kkDeg m F') := by
    intro j
    simp only [kkDeg, E']
    refine Submodule.subset_span (R := LocalOkaRing (Fin (n + 1))) ?_
    simp only [Submodule.carrier_eq_coe, Submodule.map_coe, Set.mem_image, SetLike.mem_coe]
    ...
    · simp only [polyInclPi, LinearMap.coe_piMap]
      ...
      rfl
  rw [hG']
  refine Submodule.add_mem (kkDeg m F') hH ?_
  apply Submodule.sum_mem (kkDeg m F')
  intro i hi
