...
Since a Noetherian local ring is faithfully flat in its own completion, the corollary is that
...
it: a germ that lies in the ideal generated by finitely many germs *formally* lies in it already.
...
local, so it is faithfully flat in its own completion — `AdicCompletion.flat_of_isNoetherian`
...
/-- **A germ that lies in an ideal of germs formally lies in it already**: membership of the
ideal generated by germs in `ℂ⟦X⟧` is no weaker than membership in `𝒪`.
