...
The zero locus is closed because the locus where the germ of a section is *invertible* is open —
that is `AlgebraicGeometry.RingedSpace.basicOpen`, which is an `Opens` by construction — and the
zero locus is the complement of the union of those loci.
...
This is the underlying set of the closed subspace that the `f i` cut out. Stating it by
non-invertibility of germs rather than by pointwise vanishing is what makes it available on an
arbitrary locally ringed space, where sections have no values. -/
def zeroLocus : Set Y :=
