...
**The file states the division once, in its general form.**
`LocalOkaRing.quotientDegreeOneEquiv` quotients by an arbitrary local Weierstrass polynomial of
degree one, and the three coordinate results named above —
`LocalOkaRing.exists_eq_mul_lastVar_add_incl`, `LocalOkaRing.incl_eq_zero_of_mem_span_lastVar` and
`LocalOkaRing.quotientLastVarEquiv` — are two-line corollaries of it at `q = X`, the bridge being
`LocalOkaRing.fromPolynomial_X`. Their statements are unchanged, because the induction below and
`LocalOkaRing.span_fromPolynomial_X_sub_C_ne_span_lastVar` read them.
...
- `LocalOkaRing.quotientDegreeOneEquiv`: **`LocalOkaRing (Fin (n + 1)) ⧸ (q) ≃+* LocalOkaRing
  (Fin n)` for an arbitrary local Weierstrass polynomial `q` of degree one**, and not only for the
  coordinate `X_n`. Its two halves are
  `LocalOkaRing.exists_eq_mul_fromPolynomial_add_incl` and
  `LocalOkaRing.incl_eq_zero_of_mem_span_fromPolynomial`.
- `LocalOkaRing.quotientLastVarEquiv`: the same at `q = X_n`, which is the form the
  Krull-dimension induction consumes. It and its two halves,
  `LocalOkaRing.exists_eq_mul_lastVar_add_incl` and
  `LocalOkaRing.incl_eq_zero_of_mem_span_lastVar`, are corollaries of the three above.
...
**Weierstrass division is done here once, for an arbitrary local Weierstrass polynomial of degree
one; the coordinate `X_n` is the special case in the section below.** Nothing in the argument uses
`X` beyond its degree: `localweierstrass_division` is already stated for a general Weierstrass
polynomial `q`, and at `q.degree = 1` its remainder has degree `< 1` and is therefore a constant —
which is the whole of what makes the quotient the germ ring one variable down.

**The general form comes first because the coordinate case is derived from it**, and the three
`LocalOkaRing.lastVar` results below are two-line corollaries of the three here. It is still the
coordinate case that the Krull-dimension induction consumes, so its statement is the one that must
not move; what moved is its proof.
...
**Producing the `c` from a general `f` is the last section of this file**, and two things about
it are worth saying here because the obvious statement of them is wrong. The hypothesis one
expects — that the last partial derivative is a unit — **is not expressible on `LocalOkaRing`,
which carries no partial-derivative operator**; the workable form is that the restriction to the
last axis has a simple zero, which is what `localweierstrass_preparation` computes on its own
second line. And the route is **three steps and not two**: preparation gives
`f = fromPolynomial g * u` with `u` a unit but **does not report `g.degree`**, so the degree has
to be recovered from the conclusion before a unit factor not changing the ideal can finish it.
...
`LocalOkaRing.quotientLastVarEquiv` below is this at `q = X`, transported across
`LocalOkaRing.fromPolynomial_X`. Surjectivity is
...
**The three results here are the section above at `q = X`, and each is two lines**: the last
coordinate is a degree-one local Weierstrass polynomial
(`LocalOkaRing.isLocalWeierstrassPolynomial_X`), and `LocalOkaRing.fromPolynomial_X` identifies
`fromPolynomial X` with `LocalOkaRing.lastVar`. Their *statements* are unchanged and deliberately
so: they are what the Krull-dimension induction and
`LocalOkaRing.span_fromPolynomial_X_sub_C_ne_span_lastVar` consume, and a corollary is only a
de-duplication if nothing downstream can tell.

`LocalOkaRing.quotientLastVarEquiv` needs one thing the other two do not, `Ideal.quotEquivOfEq` on
the right: the ideals `Ideal.span {fromPolynomial X}` and `Ideal.span {lastVar}` are equal, but
they are not the same term, so the two quotient *types* are not the same type. -/
...
theorem exists_eq_mul_lastVar_add_incl (f : LocalOkaRing (Fin (n + 1))) :
  ...
  obtain ⟨a, c, h⟩ := exists_eq_mul_fromPolynomial_add_incl isLocalWeierstrassPolynomial_X
    (degree_X (R := LocalOkaRing (Fin n))) f
  exact ⟨a, c, by rwa [fromPolynomial_X] at h⟩
...
theorem incl_eq_zero_of_mem_span_lastVar {c : LocalOkaRing (Fin n)}
    (h : incl c ∈ Ideal.span {(lastVar : LocalOkaRing (Fin (n + 1)))}) : c = 0 :=
  incl_eq_zero_of_mem_span_fromPolynomial isLocalWeierstrassPolynomial_X
    (degree_X (R := LocalOkaRing (Fin n))) (by rwa [fromPolynomial_X])
...
noncomputable def quotientLastVarEquiv : LocalOkaRing (Fin n) ≃+*
  ...
  (quotientDegreeOneEquiv isLocalWeierstrassPolynomial_X
    (degree_X (R := LocalOkaRing (Fin n)))).trans
    (Ideal.quotEquivOfEq (by rw [fromPolynomial_X]))

/-! ### The graph of an analytic function

A degree-one local Weierstrass polynomial in `X_n` is `X_n - c` for a germ `c` in the first `n`
variables vanishing at the origin — that is `LocalOkaRing.eq_X_sub_C_of_natDegree_eq_one` in the
section below — so the two sections above quotient by the ideal cut out by the **graph** of `c`.
The last result here is what says that this is not the coordinate case in disguise. -/
