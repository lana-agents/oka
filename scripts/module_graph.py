...
`Oka.lean` is the module `mk_all` generates and imports every module of the library; `OkaTest.lean`
is its test-side twin.  Each is therefore downstream of everything it covers, and **every file
under `OkaTest/` imports `Oka`**, so the roots conduct: without them in the graph, no module under
`OkaTest/` is downstream of any module under `Oka/` at all, and the downstream counts
`OkaTest/Axioms.lean` publishes of its own would be wrong by a factor of three.  The walk therefore
runs over the whole graph, roots included, and the **two roots alone** are subtracted from what is
reported.  `--include-aggregators` puts them back; on the six figures `OkaTest/Axioms.lean` states
of its own, that flag adds 2 to each subject under `Oka/` and 1 to each subject under `OkaTest/`
...
example rather than a warning.  `OkaTest/Axioms.lean` records that `IsProper` occurs in the code
of **no** module of this repository, and `--grep IsProper` at `0b2759b` returns **3** — all three
`IsProperMap`, which is a different class and is in six modules.  A verdict taken from the wrong
one of the two reads *the class this file declines to declare is consumed downstream* off the
class it declines in favour of.  Whichever a figure is, say which.
"""
...
def self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        files = [
            ...
            "OkaTest.lean", "OkaTest/Probe.lean",
        ]
        g = Graph(root, files)
        ...
        # The `IsProper` / `IsProperMap` pair, which is live in this tree.
        write("Oka/Wider.lean", "/-! W. -/\ndef TokenWider := 2\n")
        gw = Graph(root, files + ["Oka/Wider.lean"])
        ...
        check("--word still matches a dotted prefix",
              gw.mentions("Oka.Wider", "TokenWider", word=True), True)
