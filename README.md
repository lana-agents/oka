...
* **Local isomorphisms and finite étale morphisms** (`Oka/AnalyticSpace/LocalIso.lean`), the
  ...
  also closed, and `ℂ²` is connected. **The stalk half of the definition is exercised, by a map
  that is not an isomorphism.** `ComplexAnalytic.sq`, the squaring map of `ℂ ∖ {0}`, is in that
  file — it needed `ComplexAnalytic.AnalyticSpace.liftRestrict`, since nothing before produced a
  morphism whose *target* is an open subspace — it is proved not injective and, separately,
  **not an isomorphism** (`ComplexAnalytic.not_isIso_sq`), and
  `ComplexAnalytic.isIso_stalkMap_sq` proves **every one of its stalk maps is an isomorphism**:
  the first isomorphism of stalks here that is neither an identity nor a field of an
  `IsCutOutBy`. What makes that cheap is
  `ComplexAnalytic.isIso_stalkMap_okaMapHom` (`Oka/AnalyticSpace/StalkLocalInverse.lean`), the
  **germ dictionary**: the stalk map of a holomorphic map is precomposition of germs, so a map
  with an analytic local inverse near a point is an isomorphism on the stalk there. The local
  inverse itself is Mathlib's `AnalyticAt.analyticAt_localInverse` and **no branch of the square
  root is built here.** *Its finiteness and the topological field of `IsLocalIso` are still not
  proved*, so `IsLocalIso sq` is not stated. Neither `IsCoveringMap` nor the theorem that a finite
  local isomorphism onto a connected base is one appears; that is a third rung.
