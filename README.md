...
**Since 2026-09-16 it also reads the `…`-elided citations**, which are this repository's house
...
no such defect — **230 occurrences at `a5bb009`**, the base this paragraph is written against,
where `scripts/check_docstring_names.py` pins **207** at `daeb942`, the base the branch was cut
from. **Both numerals name a commit that has already happened, which is what makes them
