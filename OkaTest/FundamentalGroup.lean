...
* **Nothing about a second base, and what the witness has to be is *connected*.** Covers that are
  ...
  `…SeparatedFiniteEtaleOver.nonempty_iso_id` asks of a cover and the trivial ones do not have is
  `CategoryTheory.PreGaloisCategory.IsConnected`. **`sqOver` is the only object here that a
  statement of this repository says is not the base over itself** — `IsEmpty (… ≅ …)` occurs
  **six** times in the comment-stripped code under `Oka/` and `OkaTest/`, four of them universally
  quantified in `Oka/AnalyticSpace/FiniteEtaleOver.lean` and two about `sqOver`, of which
  `OkaTest.FiniteEtaleOver.not_iso_id_sqOver` is the one whose other side is the base over itself
  — and that, and not the absence of non-trivial covers elsewhere, is what makes the punctured
  line the one base this argument can run at.
-/
