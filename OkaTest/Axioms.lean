...
`Oka/Geometry/RingedSpace/`**, which is the spelling that file's own docstring had been repaired
out of by lana-agents/oka#478, and it is repaired here to the naming form that push installed
there. **A pin does not exempt a proportion the way it exempts a figure**, and that is the second
thing this widening settles: a reader can re-run a figure at the commit it is pinned to and cannot
re-run *nearly all of them* at any commit, because the population has to be classified by hand
first, which is the objection above.
