...
**The five grep-decidable verdicts of the third and fourth sweeps were re-run at this head, and all
five reproduce with their sets grown.** `Oka/Analytification/AffineCover.lean`: the four tokens in
the code of **0** of **130** downstream modules, against 124 then.
...
of it.** `--grep IsProper` returns **3** at this head — `OkaTest/Axioms/Morphisms.lean`,
...
**The decidable pair column was re-run mechanically at this head, and it is clean.** Of the **338**
...
opening *The 21 is `d2ae161`'s* predicted of any sweep that quotes what it swept. At the head that
writes it the three-column partition is **719 / 344 / 170 / 205** against `0b2759b`'s **702 / 338 /
167 / 197**, and the downstream-set pattern over the whole population is **46 / 24** against
**44 / 22** — **per-file identical in every other file of the 355**, the whole of both deltas being
this file's own count going **62 → 79** and **22 → 24**. **The residue's two narrow columns are
`0b2759b`'s 17 and 20**; at this head the importer one is **25**, the added ones being this block's
quotations, and the downstream-set one is unmoved at **17**, because what this block adds to that
pattern names a module in the sentence and so is not in the residue at all. **A sixth sweep should
take 44 as `0b2759b`'s**, re-derive its own at its own head, and expect to have raised it again.
