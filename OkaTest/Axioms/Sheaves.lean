...
this module's convention rather than an omission: it already carried six equation lemmas before
...
`AlgebraicGeometry.LocallyRingedSpace.stalkToInverseImageSheafIso`,
`AlgebraicGeometry.LocallyRingedSpace.toInverseImageSheaf` and
`AlgebraicGeometry.LocallyRingedSpace.sheetToBase`, and `git grep` finds a `#print axioms` for
none of the six. -/
