...
`ComplexAnalytic.exists_index_mvPolynomial_basicOpen_specSchemeIotaMap` — guarded in the section
above — with `choose` applied to it, so `Classical.choice` is what the step *is* rather than
