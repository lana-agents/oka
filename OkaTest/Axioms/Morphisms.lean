...
**Outside this clause and the section it names, this push edits only the two section docstrings of
this file that carried the retired sentence**, which `git diff` against the commit it is cut from
shows. **The four counts the clause opening *A section beyond those, at the
